defmodule Prismatic.LowerSimulationScenario do
  @moduledoc """
  Phase 6 lower simulation scenario declaration for Prismatic GraphQL transport.

  Prismatic owns the GraphQL lower scenario surface. Selection remains an owner
  ...
  defp reject_semantic_provider_policy!(attrs) do
    if Enum.any?(@forbidden_semantic_keys, &has_key?(attrs, &1)) do
      raise ArgumentError,
            "semantic provider policy must not be owned by Prismatic lower scenarios"
    end
  end
