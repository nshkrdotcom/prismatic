defmodule Prismatic.OAuth2.InteractiveTest do
  use ExUnit.Case, async: true
  import Mox

  alias Prismatic.OAuth2
  alias Prismatic.OAuth2.Error
  alias Prismatic.OAuth2.Interactive
  ...
  test "accepts a full redirect url pasted manually" do
    expect(Prismatic.OAuthHTTPClientMock, :request, fn opts ->
      assert opts[:url] == "https://api.example.com/oauth/token"
      ...
      {:ok,
       %{
         ...
         body: %{"access_token" => "secret_access", "token_type" => "bearer"}
       }}
    end)
